...
namespace USEPA_Simulation_PlugIns
{
    ...
    public partial class GreenScopeForm : Form
    {
        ...
        private System.Collections.Generic.List<CAPEOPEN.ICapeIdentification> p_Units;
        ...
        public GreenScopeForm(ICapeFlowsheetMonitoring monitoring)
        {
            ...
            p_Units = new System.Collections.Generic.List<CAPEOPEN.ICapeIdentification>();
        ...
        void PutUnitOpsInDataTable(List<CAPEOPEN.ICapeIdentification> units, DataTable table)
        {
            table.Rows.Clear();
            foreach (CAPEOPEN.ICapeIdentification p_Unit in units)
            {
        ...
        void PutStreamsInDataTable(List<CAPEOPEN.ICapeIdentification> streams, DataTable table)
        {
            table.Rows.Clear();
            foreach (CAPEOPEN.ICapeIdentification p_Stream in streams)
            {
                if (p_Stream is CAPEOPEN.ICapeThermoMaterial)
                {
                    ...
                    if (VaporPresent)
                    {
                        p_TMO.GetSinglePhaseProp("phaseFraction", "vapor", "mole", ref obj);
                        fraction = (double[])obj;
                    }
                    row["MVF"] = fraction[0];
                    ...
                    CAPEOPEN.ICapeThermoMaterial material = p_TMO.CreateMaterial();
                    material.CopyFromMaterial(p_TMO);
                    material.GetPresentPhases(ref obj, ref obj1);
                    string[] phases = (string[])obj;
                    CAPEOPEN.eCapePhaseStatus[] statuses = (CAPEOPEN.eCapePhaseStatus[])obj1;
                    ...
                    {
                        if (statuses[i] == CAPEOPEN.eCapePhaseStatus.CAPE_ATEQUILIBRIUM || statuses[i] == CAPEOPEN.eCapePhaseStatus.CAPE_ESTIMATES)
                        {
                            ((CAPEOPEN.ICapeThermoPropertyRoutine)(material)).CalcSinglePhaseProp(props, phases[i]);
        ...
        void UpdateStreams()
        {
            ...
            foreach (CAPEOPEN.ICapeIdentification p_Stream in p_InletStreams)
            {
                if (p_Stream is CAPEOPEN.ICapeThermoMaterial)
                {
                    CAPEOPEN.ICapeThermoCompounds p_Compounds = (CAPEOPEN.ICapeThermoCompounds)p_Stream;
                    object obj1 = null;
                    object obj2 = null;
                    object obj3 = null;
                    object obj4 = null;
        ...
        void ResetStreams()
        {
            ...
            for (int i = 0; i < p_UnitColl.Count(); i++)
            {
                ...
                {
                    p_Units.Add((CAPEOPEN.ICapeIdentification)p_Unit);
        ...
        private void AddInputStreamsToSpreadsheet(DocumentFormat.OpenXml.Packaging.SpreadsheetDocument spreadsheet, System.Collections.Generic.List<CAPEOPEN.ICapeIdentification> streams)
        {
            ...
            for (int i = 0; i < streams.Count; i++)
            {
                ...
                //object temp = null;
                //((CAPEOPEN.ICapeThermoMaterial)streams[i]).GetOverallProp("totalFlow", "Mass", temp);
                ...
                //}
                //((CAPEOPEN.ICapeThermoMaterial)streams[i]).GetOverallProp("temperature", "UNDEFINED", temp);
                //SetSpreadsheetCellValue(worksheetPart.Worksheet, componentColumns[i], 83, ((double[])temp)[0]-273.15);
                //((CAPEOPEN.ICapeThermoMaterial)streams[i]).GetOverallProp("pressure", "UNDEFINED", temp);
                //SetSpreadsheetCellValue(worksheetPart.Worksheet, componentColumns[i], 84, ((double[])temp)[0]);
                //((CAPEOPEN.ICapeThermoMaterial)streams[i]).GetOverallProp("fraction", "vapor", temp);
                //SetSpreadsheetCellValue(worksheetPart.Worksheet, componentColumns[i], 85, ((double[])temp)[0]);
                //((CAPEOPEN.ICapeThermoMaterial)streams[i]).GetOverallProp("enthalpy", "UNDEFINED", temp);
                //SetSpreadsheetCellValue(worksheetPart.Worksheet, componentColumns[i], 86, ((double[])temp)[0] * 3600);//streams[i].EnthalpyMJHR);
                //((CAPEOPEN.ICapeThermoMaterial)streams[i]).GetOverallProp("entropy", "UNDEFINED", temp);
