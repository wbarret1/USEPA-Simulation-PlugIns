...
namespace USEPA_Simulation_PlugIns
{
    ...
    public partial class WARalgorithm : Form
    {
        ...
        private System.Collections.Generic.List<USEPA_Simulation_PlugIns.ICapeUnitCOM> p_Units;
        ...
        public WARalgorithm(System.Data.DataTable warDataTable, ICapeFlowsheetMonitoring Monitoring)
        {
            ...
            {
                InitializeComponent();
                if (Monitoring.ValStatus != USEPA_Simulation_PlugIns.CapeValidationStatus.CAPE_VALID)
                {
        ...
        void ResetStreams()
        {
            ...
            USEPA_Simulation_PlugIns.ICapeCollection p_UnitColl = (ICapeCollection)p_Monitoring.GetUnitOperationCollection();
            USEPA_Simulation_PlugIns.ICapeCollection p_StreamColl = (ICapeCollection)p_Monitoring.GetStreamCollection();
            //
        ...
        private void AvailableUnitOpsheckedListBox_ItemCheck(object sender, ItemCheckEventArgs e)
        {
            try
            {
                if (!starting)
                {
                    ...
                    {
                        if (((USEPA_Simulation_PlugIns.ICapeIdentification)unit).ComponentName == name)
                        {
                            changedUnit = unit;
                        }
                        else
                    ...
                    if (e.NewValue == System.Windows.Forms.CheckState.Unchecked)
                    {
                        for (int j = 0; j < p_UnitPortColl.Count(); j++)
                        {
                            ...
                            if (p_Id != null)
                            {
                                USEPA_Simulation_PlugIns.ICapeIdentification p_Outlet = null;
                                USEPA_Simulation_PlugIns.ICapeIdentification p_Inlet = null;
                                foreach (ICapeIdentification stream in p_OutletStreams)
                                {
                                    if (stream.ComponentName == p_Id.ComponentName)
                                    {
