...
namespace USEPA_Simulation_PlugIns
{
    ...
    public partial class UnitSelector : Form
    {
        ...
        public UnitSelector()
        {
            ...
            node = null;
            String USEPA_Simulation_PlugInsDirectory = String.Concat(Environment.GetEnvironmentVariable("CommonProgramFiles"), "\\CAPE-OPEN Objects");
            if (System.IO.Directory.Exists(USEPA_Simulation_PlugInsDirectory))
            {
                node = new TreeNode("USEPA_Simulation_PlugIns Units");
                this.treeView1.Nodes.Add(node);
                AddPath(USEPA_Simulation_PlugInsDirectory, node);
            }
            USEPA_Simulation_PlugInsDirectory = "C:\\CAPE-OPEN Objects";
            if (System.IO.Directory.Exists(USEPA_Simulation_PlugInsDirectory))
            {
                if (node == null)
                {
        ...
        private void AddType(Type type, TreeNode node)
        {
            if (!type.IsAbstract)
            {
                if (typeof(USEPA_Simulation_PlugIns.ICapeUnit).IsAssignableFrom(type))
                {
                    if (type.FullName != "USEPA_Simulation_PlugIns.UnitOperationManager")
                    {
        ...
        {
            throw new USEPA_Simulation_PlugIns.CapeUnknownException("Unit Creation Cancelled.");
            // this.Close();
        }
